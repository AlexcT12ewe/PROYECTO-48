...
var pizzaImage, pizza, pizzaGrupo;
var corazon, corazonImage;
...
function preload(){
  ...
  pizzaImage = loadImage("Imagenes/pizza.png");
  corazonImage = loadImage("Imagenes/life.png");
...
function creacionPizaa(){
  if(frameCount % 150 === 0) {
    //crear los elementos pizza
    pizza = createSprite(width,660,20,30);
    pizza.addImage(pizzaImage);
    pizza.scale = 0.5;
    pizza.velocityX = -3;
    
    pizzaGrupo.add(pizza);
    ...
    //creacion de corazon
    corazon = createSprite(290,50,20,20);
    corazon.addImage(corazonImage);
    corazon.scale = 0.2;
    corazon.visible = false;
    
}

function creacionVidas(){
  if (dib.isTouching(pizza)){
    console.log("si entra a la estructura if");
